...
class CompraUpdateView(UpdateView):
    ...
    def get_details_product(self):
        data = []
        try:
            for i in DetalleCompra.objects.filter(compra_id=self.kwargs.get('pk')):
                item = i.producto.toJSON()
                item['cantidad'] = i.cantidad
                item['precio_compra'] = i.precioCompra
                data.append(item)
        except:
            pass
        return data
    ...
    def get_context_data(self, **kwargs):
        print('updated')
        context=super().get_context_data(**kwargs)
        context['url'] = reverse_lazy('compra:updCompra',kwargs={'pk': self.kwargs.get('pk')})
        context['action'] = 'edit'
